...
const build = () => execFileSync('node', ['scripts/build-vocab-data.mjs'], { cwd: new URL('..', import.meta.url) });
...
describe('vocabulary build outputs', () => {
  it('stamps the service worker cache with a content hash', () => {
    ...
    const src = new URL('../src/vocab/learn.core.js', import.meta.url);
  ...
  it('does not modify its own sources', () => {
    // The previous build rewrote public/deutsch-woerter/learn.js in place through
    ...
    const hash = () => sources.map((f) => createHash('sha256')
      .update(readFileSync(new URL(`../src/vocab/${f}`, import.meta.url)))
      .digest('hex')).join();
    const before = hash();
    build();
